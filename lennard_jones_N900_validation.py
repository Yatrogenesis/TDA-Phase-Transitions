#!/usr/bin/env python3
"""
Lennard-Jones 2D Phase Transition - N=900 VALIDATION
=====================================================

Finite-size validation with N=900 particles.
Expected: Higher precursor rate as fluctuations decrease further.
...
STEPS_PROD = 5000
SAMPLE_INTERVAL = 25
N_TRIALS = 5              # Reduced for N=900 (much slower)
THERMOSTAT_TAU = 50
...
def hexatic_order_pbc(pos, box):
    """Compute hexatic order parameter |psi_6|."""
    N = len(pos)
    psi6_mags = []
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("LENNARD-JONES 2D - N=900 VALIDATION")
    print("=" * 70)
    ...
    # Run trials
    print(f"\n1. Running {N_TRIALS} trials (N=900 is ~15x slower than N=144)...")
    results = []
    start_time = time.time()

    for i in range(N_TRIALS):
        seed = 9000 + i  # Unique seeds for N=900
        t0 = time.time()
        result = run_trial(seed)
        results.append(result)
        elapsed_trial = time.time() - t0
        print(f"   Trial {i+1}/{N_TRIALS}: {result.final_phase}, |psi6|={result.psi6_final:.3f}, gap={result.gap_cusum} ({elapsed_trial:.1f}s)")
    ...
    # Analysis
    print("\n" + "=" * 70)
    print("N=900 VALIDATION RESULTS")
    print("=" * 70)
    ...
    if n_crystal > 0:
        ...
        if n_detected > 0:
            ...
            print(f"N=400: 80.0% precursor rate, 1385.0 mean gap")
            print(f"N=900: {100*precursor_rate:.1f}% precursor rate, {np.mean(gaps_cusum):.1f} mean gap")

            if precursor_rate > 0.80:
                print("IMPROVED: Larger system shows even higher precursor rate")
            elif precursor_rate >= 0.70:
                print("CONSISTENT: Results show robust finite-size behavior")
            else:
                print("? INVESTIGATE: May indicate polycrystallization")
    ...
    if crystal_trials:
        ...
        axs[0].plot(ref_times, psi6_ens, 'm-', lw=2, label='|psi_6|')
        axs[0].fill_between(ref_times, psi6_ens - psi6_std, psi6_ens + psi6_std, alpha=0.3, color='m')
        axs[0].axhline(0.5, ls='--', color='gray')
        axs[0].set_ylabel('|psi_6|')
        axs[0].set_title(f'N=900 Validation ({n_crystal} crystal trials)', fontweight='bold')
        axs[0].legend()
        ...
        plt.tight_layout()
        plt.savefig(f'{OUTPUT_DIR}/N900_ensemble.png', dpi=150)
        print(f"\nSaved: {OUTPUT_DIR}/N900_ensemble.png")
        plt.close()
    ...
    summary = {
        ...
        'results': {
            ...
            'cusum_method': {
                ...
                'precursor_rate': float(precursor_rate) if n_detected > 0 else None,
        ...
        'comparison': {
            'n144': {'precursor_rate': 0.733, 'mean_gap': 750.8},
            'n400': {'precursor_rate': 0.80, 'mean_gap': 1385.0}
        }
    }

    with open(f'{OUTPUT_DIR}/validation_N900.json', 'w') as f:
        json.dump(summary, f, indent=2)
    print(f"Saved: {OUTPUT_DIR}/validation_N900.json")

    print("\n" + "=" * 70)
    print("N=900 VALIDATION COMPLETE")
    print("=" * 70)
