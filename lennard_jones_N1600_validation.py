#!/usr/bin/env python3
"""
Lennard-Jones 2D Phase Transition - N=1600 VALIDATION
======================================================

Finite-size validation with N=1600 particles.
Largest system size - provides strongest finite-size test.
...
STEPS_PROD = 5000
SAMPLE_INTERVAL = 25
N_TRIALS = 3              # Minimal trials for N=1600 (very slow)
THERMOSTAT_TAU = 50
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("LENNARD-JONES 2D - N=1600 VALIDATION")
    print("=" * 70)
    ...
    # Run trials
    print(f"\n1. Running {N_TRIALS} trials (N=1600 is ~30x slower than N=144)...")
    results = []
    start_time = time.time()

    for i in range(N_TRIALS):
        seed = 16000 + i  # Unique seeds for N=1600
        t0 = time.time()
    ...
    # Analysis
    print("\n" + "=" * 70)
    print("N=1600 VALIDATION RESULTS")
    print("=" * 70)
    ...
    if n_crystal > 0:
        ...
        if n_detected > 0:
            ...
            print(f"N=900:  (pending)")
            print(f"N=1600: {100*precursor_rate:.1f}% precursor rate, {np.mean(gaps_cusum):.1f} mean gap")

            if precursor_rate >= 0.70:
                print("\nCONSISTENT: TDA-CUSUM robust across system sizes")
            else:
                print("\n? NOTE: Large systems may show polycrystallization")
    ...
    if crystal_trials:
        ...
        axs[0].set_title(f'N=1600 Validation ({n_crystal} crystal trials)', fontweight='bold')
        axs[0].legend()
        ...
        plt.tight_layout()
        plt.savefig(f'{OUTPUT_DIR}/N1600_ensemble.png', dpi=150)
        print(f"\nSaved: {OUTPUT_DIR}/N1600_ensemble.png")
        plt.close()
    ...
    with open(f'{OUTPUT_DIR}/validation_N1600.json', 'w') as f:
        json.dump(summary, f, indent=2)
    print(f"Saved: {OUTPUT_DIR}/validation_N1600.json")

    print("\n" + "=" * 70)
    print("N=1600 VALIDATION COMPLETE")
    print("=" * 70)
