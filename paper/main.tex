...
Our study employs $N=144$ particles, a system size chosen to balance computational tractability with statistical power across 30 independent trials. While this permits robust ensemble statistics, finite-size effects merit discussion.

In 2D systems, the correlation length $\xi$ near the transition can approach the system size $L \approx 12\sigma$, potentially influencing fluctuation statistics. However, several observations support the validity of our conclusions: (i) the $|\psi_6|$ transition is sharp and reproducible across trials, indicating the system clearly distinguishes liquid from crystalline phases; (ii) the CUSUM detection threshold ($3\sigma$) was calibrated against liquid-phase baselines, making results robust to absolute entropy magnitudes; and (iii) the 73.3\% precursor rate significantly exceeds chance (50\%), with $p < 0.02$ by binomial test.

Extrapolation to larger systems ($N = 400, 900$) would reduce relative fluctuations as $\sim 1/\sqrt{N}$ and permit finer temporal resolution of the transition process. We anticipate that the precursor rate may increase with system size as the separation between topological instability and metric consolidation becomes more pronounced. Systematic finite-size scaling remains a natural direction for future work.
