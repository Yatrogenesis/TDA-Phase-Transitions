...
\textbf{Three-dimensional systems.} Extension to 3D Lennard-Jones would enable analysis of both $H_1$ (loops) and $H_2$ (voids/cavities) homology groups. The richer topological structure may reveal additional precursor signatures, particularly through void coalescence dynamics during crystallization. Preliminary work with $N=256$ particles shows 100\% crystallization with $Q_6 \approx 0.95$ (FCC/HCP structure).
...
\textbf{Glass-forming systems.} The Kob-Andersen binary mixture (80\% A, 20\% B with size disparity) provides an ideal test system for vitrification. Preliminary studies across five quench rates ($10^9$--$10^{14}$ K/s equivalent) show 100\% vitrification with low $Q_6 \approx 0.09$ (disordered structure), confirming that size frustration effectively suppresses crystallization. The relationship between quench rate and glass relaxation (measured via MSD) suggests that TDA may distinguish well-relaxed from poorly-relaxed glasses.
