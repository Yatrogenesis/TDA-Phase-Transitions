...
Our primary study employs $N=144$ particles, a system size chosen to balance computational tractability with statistical power across 30 independent trials. To validate that our conclusions are not artifacts of finite-size effects, we performed additional simulations with $N=400$ particles (10 independent trials).

The $N=400$ validation yielded a precursor rate of 80.0\% (8/10 trials), compared to 73.3\% for $N=144$. Notably, the mean temporal gap increased substantially from 750.8 to 1385.0 integration steps, while the relative standard deviation decreased. These results confirm that the topological precursor signal strengthens with increasing system size, consistent with the expected reduction of thermal fluctuations as $\sim 1/\sqrt{N}$.

\begin{table}[h]
\centering
\caption{Finite-size scaling of precursor detection.}
\label{tab:finitesize}
\begin{tabular}{lccc}
\hline
$N$ & Precursor Rate & Mean Gap (steps) & $\sigma$/Mean \\
\hline
144 & 73.3\% & 750.8 $\pm$ 1041.3 & 1.39 \\
400 & 80.0\% & 1385.0 $\pm$ 893.4 & 0.65 \\
\hline
\end{tabular}
\end{table}

The improvement in both precursor rate and signal-to-noise ratio with increasing $N$ supports the physical reality of the topological precedence phenomenon. The 73.3\% rate at $N=144$ significantly exceeds chance (50\%), with $p < 0.02$ by binomial test, and the $N=400$ results further reinforce this conclusion.
...
The methodology developed here---TDA-CUSUM detection of topological precursors---opens several avenues for extension to more complex systems:

\textbf{Three-dimensional systems.} Extension to 3D Lennard-Jones would enable analysis of both $H_1$ (loops) and $H_2$ (voids/cavities) homology groups. The richer topological structure may reveal additional precursor signatures, particularly through void coalescence dynamics during crystallization.

\textbf{Molecular systems.} Application to realistic water models (e.g., TIP4P) would test whether hydrogen bond network topology exhibits similar precursor behavior during ice nucleation. The tetrahedral coordination of water suggests distinctive $H_1$ signatures compared to simple liquids.

\textbf{Glass-forming systems.} Rapid quenching of Lennard-Jones systems into the glassy regime (avoiding crystallization) would probe whether topological entropy dynamics differ fundamentally between crystallization and vitrification pathways. The absence of long-range order in glasses may manifest as persistent topological complexity.

\textbf{Larger system sizes.} Preliminary validation with $N=900$ and $N=1600$ particles (see Supplementary Information) demonstrates that precursor detection improves with system size, achieving 100\% detection rates. Full statistical analysis of these larger systems would strengthen finite-size scaling conclusions.

These extensions would establish whether stochastic informational primacy represents a universal feature of first-order phase transitions or is specific to particular system classes.
